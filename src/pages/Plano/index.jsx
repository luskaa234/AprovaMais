...
function statusTone(status) {
  const tones = {
    ...
    "Em andamento": "border-blue-200 bg-royal/10 text-navy",
...
function ActivityRow({ activity, onStatus, onOpen, onEdit, onDelete }) {
  ...
  return (
    <div data-generated={activity.generated ? "true" : undefined} className={cx("plano-activity-row relative overflow-hidden rounded-lg border bg-white p-3 shadow-sm transition hover:shadow-md", isDone ? "border-emerald-100" : "border-slate-100 hover:border-blue-200")}>
      <span className={cx("absolute left-0 top-0 h-full w-1", typeTone(activity.type))} />
      <div className="plano-activity-grid grid gap-3 pl-2">
        <button
          aria-label={isDone ? "Marcar como pendente" : "Marcar como concluida"}
          className={cx("plano-activity-check grid size-10 place-items-center rounded-lg border transition", isDone ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-royal/20 bg-white text-slate-400 hover:text-navy")}
          onClick={() => onStatus(activity.id, isDone ? "Pendente" : "Concluida")}
          type="button"
        >
          <CheckCircle2 size={20} />
        </button>
        <div className={cx("plano-activity-time rounded-lg border px-3 py-2", isDone ? "border-emerald-100 bg-emerald-50 text-emerald-800" : "border-royal/20 bg-royal/10 text-blue-800")}>
        ...
        <div className="plano-activity-copy min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button className="min-w-0 text-left" onClick={() => onOpen(activity)} type="button">
              <h3 className="plano-activity-title text-base font-black text-slate-950">{activity.title}</h3>
            </button>
          </div>
          <p className="mt-1 text-sm text-slate-500">{activity.materia} · {activity.concurso}</p>
          <div className="plano-activity-progress mt-3 max-w-sm">
            <div className="flex items-center justify-between gap-3 text-xs">
              <span className={cx("font-black", isRunning ? "text-navy" : activity.elapsedSeconds ? "text-slate-600" : "text-slate-400")}>
                Cronômetro {formatElapsed(activity.elapsedSeconds)}
              </span>
              {isRunning ? <span className="rounded-full bg-royal/15 px-2 py-0.5 font-bold text-navy">rodando</span> : null}
            </div>
            <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-slate-100">
              <div className={cx("h-full rounded-full transition-all", isDone ? "bg-emerald-500" : isRunning ? "bg-royal" : "bg-slate-300")} style={{ width: `${timerProgress}%` }} />
            </div>
          </div>
          <div className="plano-activity-mobile-badges mt-3 flex flex-wrap gap-2 xl:hidden">
        ...
        <div className="plano-activity-controls flex flex-wrap items-center gap-2 md:justify-end">
          <div className="plano-activity-desktop-badges hidden flex-wrap items-center gap-2 md:flex">
            <Badge variant={typeBadge(activity.type)}>{activity.type}</Badge>
          ...
          <div className="plano-activity-actions flex flex-wrap gap-2">
            <button className={cx("inline-flex min-h-9 items-center gap-1.5 rounded-lg border px-3 text-xs font-bold transition", isRunning ? "border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100" : "border-royal/20 bg-royal/10 text-navy hover:bg-royal/15")} onClick={() => onStatus(activity.id, isRunning ? "Pendente" : "Em andamento")} type="button">
            ...
            <button className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-royal/20 bg-white px-3 text-xs font-bold text-navy transition hover:bg-royal/10" onClick={() => onOpen(activity)} type="button">
...
export default function PlanoPage() {
  ...
  return (
    <div className="plano-page mx-auto min-h-[calc(100vh-9rem)] max-w-[1500px] overflow-visible pb-10 text-slate-900">
      <div className="plano-header mb-5 flex flex-col gap-4 rounded-lg border border-royal/20 bg-white p-4 shadow-sm xl:flex-row xl:items-center xl:justify-between" data-tour="tour-studies-header">
        <div className="flex items-center gap-3">
          <span className="grid size-11 place-items-center rounded-lg border border-royal/20 bg-white text-royal shadow-sm"><CalendarDays size={22} /></span>
          <div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="hidden items-center gap-2 rounded-lg bg-royal/10 px-3 py-2 text-sm font-bold text-navy xl:flex">{progress}% da semana</div>
      ...
      <section className="plano-personalizacao mb-4 rounded-lg border border-royal/20 bg-white p-4 shadow-sm" data-tour="tour-studies-personalization">
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          <div>
            <span className="text-xs font-black uppercase tracking-wide text-royal">Personalização ativa</span>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          {planDayOptions.map(([day, label]) => {
            ...
            return (
              <button
                key={day}
                className={cx("min-h-9 rounded-lg border px-3 text-xs font-black transition", active ? "border-royal bg-royal text-white" : "border-royal/20 bg-white text-slate-600 hover:border-sky")}
      ...
      <div className="plano-desktop-filters mb-4 hidden gap-3 rounded-lg border border-royal/20 bg-white p-4 shadow-sm xl:grid" data-tour="tour-studies-filters">
        {filtersContent}
      </div>

      <div className="plano-view-tabs mb-4 flex flex-wrap gap-2">
        {views.map((item) => (
          <button key={item} onClick={() => setView(item)} className={cx("min-h-10 rounded-lg border px-5 text-sm font-semibold transition", view === item ? "border-royal bg-royal text-white shadow-sm" : "border-royal/20 bg-white text-slate-600 hover:border-sky")}>{item}</button>
        ))}
      </div>

      <div className="plano-layout grid gap-4 xl:grid-cols-[1fr_360px]">
        <main className="plano-main space-y-4">
          <section className="plano-calendar rounded-lg border border-royal/20 bg-white shadow-sm" data-tour="tour-studies-calendar">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 p-4">
              ...
              <span className="inline-flex items-center gap-2 rounded-lg bg-royal/10 px-3 py-2 text-sm font-semibold text-navy"><Filter size={15} /> Filtros ativos</span>
            </div>
                ...
                <div className="min-w-[720px]">
                  ...
                  <div className="grid grid-cols-7">
                    {monthDays.map((date) => {
                      ...
                      return (
                        <button key={dateKey} type="button" onClick={() => setSelectedDate(dateKey)} className={cx("min-h-28 border-b border-r border-slate-100 p-3 text-left transition hover:bg-royal/10", selected && "bg-royal/10 ring-2 ring-inset ring-blue-500", !inMonth && "bg-slate-50 text-slate-400")}>
                          <span className={cx("grid size-8 place-items-center rounded-full text-sm font-bold", selected ? "bg-royal text-white" : "text-slate-700")}>{date.getDate()}</span>
          ...
          <section className="plano-activities overflow-hidden rounded-lg border border-royal/20 bg-white shadow-sm" data-tour="tour-studies-activities">
            <div className="border-b border-royal/20 bg-slate-50 px-4 py-4">
              ...
              <div className="mt-4 grid gap-2 sm:grid-cols-[1fr_auto] sm:items-center">
                <div className="h-2 overflow-hidden rounded-full bg-white ring-1 ring-royal/20">
                  <div className="h-full rounded-full bg-royal transition-all" style={{ width: `${selectedProgress}%` }} />
                </div>
                <span className="text-xs font-bold text-navy">{selectedProgress}% do dia concluído</span>
              </div>
            </div>
          ...
          <section className="plano-stats grid gap-4 rounded-lg border border-royal/20 bg-white p-4 shadow-sm md:grid-cols-5">
            {[
            ...
            ].map(([label, value]) => <div key={label} className="text-center"><strong className="block text-xl text-royal">{value}</strong><span className="text-xs text-slate-500">{label}</span></div>)}
          </section>
        </main>

        <aside className="plano-side-summary space-y-4" data-tour="tour-studies-progress">
          <section className="rounded-lg border border-royal/20 bg-white p-4 shadow-sm">
            <h2 className="font-black text-slate-950">Progresso semanal</h2>
            <div className="mt-4 flex items-center gap-4">
              <div className="grid size-24 place-items-center rounded-full border-[10px] border-royal text-xl font-black text-slate-950">{progress}%</div>
          ...
          <section className="rounded-lg border border-royal/20 bg-white p-4 shadow-sm">
            <div className="mb-3 flex items-center gap-2"><TrendingUp className="text-royal" size={18} /><h2 className="font-black text-slate-950">Sequência de estudos</h2></div>
            <strong className="text-3xl text-slate-950">{studyStreak}</strong>
            <span className="ml-2 text-sm text-slate-500">dias seguidos</span>
          </section>

          <section className="rounded-lg border border-royal/20 bg-white p-4 shadow-sm">
            <div className="mb-3 flex items-center gap-2"><Target className="text-royal" size={18} /><h2 className="font-black text-slate-950">Matérias em foco</h2></div>
            <div className="flex flex-wrap gap-2">{focus.map((item) => <span key={item} className="rounded-full bg-royal/10 px-3 py-1 text-xs font-bold text-navy">{item}</span>)}</div>
          </section>
        </aside>
      </div>
      ...
      <Modal open={modal} title={editingId ? "Editar atividade" : "Nova atividade"} onClose={() => setModal(false)} footer={<Button onClick={createActivity}>Salvar atividade</Button>}>
        <div className="grid gap-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Input label="Horário" type="time" value={draft.hour} onChange={(event) => setDraft((current) => ({ ...current, hour: event.target.value }))} />
            <Input label="Duração" type="number" value={draft.duration} onChange={(event) => setDraft((current) => ({ ...current, duration: event.target.value }))} />
