...
function splitArticleText(rawArticle, lei, chapter) {
  ...
  return sourcePieces.map((piece, index) => {
    ...
    return {
      ...
      area: lei.categoria || lei.materia || "Legislação",
      materia: lei.materia || lei.categoria || "Legislação",
      tipo: lei.tipo || "Lei",
      numero,
...
export default function LeisSecasPage() {
  ...
  const filteredLaws = useMemo(() => {
    ...
    if (chip === "edital") source = laws.filter((law) => ["Constitucional", "Penal", "Processual", "Militar", "Legislação", "Administrativo"].some((term) => normalize(law.area).includes(normalize(term)) || normalize(law.materia).includes(normalize(term))));
    if (!q) return source;
