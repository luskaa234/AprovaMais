...
export default function PixPanel({ pixData, onSuccess, onCancel }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Status badge */}
      {status === "waiting" && (
        <div className="flex items-center gap-2 rounded-xl border border-royal/20 bg-royal/10 px-3.5 py-2.5 text-sm text-navy">
          <span className="inline-block h-2 w-2 animate-pulse rounded-full bg-royal" />
      ...
      <div className="flex flex-col items-center gap-3">
        <div className="rounded-2xl border-2 border-royal/20 bg-white p-3 shadow-sm">
          <img
      ...
      {pixData.qr_code && (
        <button
          type="button"
          onClick={handleCopy}
          className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-blue-200 bg-royal/10 py-3 text-sm font-medium text-navy transition-colors hover:bg-royal/15 active:bg-blue-200"
        >
          <Copy size={15} />
