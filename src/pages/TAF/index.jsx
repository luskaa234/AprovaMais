...
const COMMON_TAF = [
  {
    title: "Barra fixa",
    detail: "Teste de força para braços, costas e pegada. Em muitos editais, a prova é dinâmica para homens e estática/isometria para mulheres.",
  },
  {
...
const TAF_WEEK_TEMPLATE = [
  ...
  { dia: "Terça", foco: "Força de empurrar e core", tipos: ["flexao", "abdominal"], bloco: "5 séries técnicas sem falhar", intensidade: "Média" },
  { dia: "Quarta", foco: "Corrida contínua", tipos: ["corrida"], bloco: "25 a 35 min em ritmo confortável", intensidade: "Média" },
  { dia: "Quinta", foco: "Barra e resistência muscular", tipos: ["barra", "flexao"], bloco: "Séries submáximas com descanso completo", intensidade: "Alta" },
  { dia: "Sexta", foco: "Abdominal e mobilidade", tipos: ["abdominal"], bloco: "Volume controlado + alongamento", intensidade: "Leve" },
  { dia: "Sábado", foco: "Simulado TAF", tipos: ["corrida", "flexao", "abdominal", "barra"], bloco: "Teste na ordem do edital e registro real", intensidade: "Teste" },
];
...
function buildSmartWeek(result = {}) {
  ...
  return TAF_WEEK_TEMPLATE.map((day) => {
    ...
    return {
      ...day,
      priority,
      label: priority ? "Prioridade" : "Manutenção",
  ...
  return (
    <div className="taf-plan-layout grid gap-4 xl:grid-cols-[1.25fr_0.75fr]">
      <Card>
        ...
        <div className="taf-progress-grid mb-4">
          {progress.map((item) => (
            <div key={item.tipo} className="taf-progress-card rounded-lg border border-royal/20 bg-white p-3 text-slate-900">
              <div className="taf-progress-card-head">
        ...
        <div className="mb-4 grid gap-3">
          {smartWeek.map((day) => (
            <div key={day.dia} className="rounded-lg border border-royal/20 bg-slate-50 p-3">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex flex-wrap gap-1">
                  {day.progress.map((item) => <span key={item.tipo} className="rounded-full bg-royal/10 px-2 py-1 text-xs font-bold text-navy">{item.label} {item.percent}%</span>)}
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="taf-secondary-panel grid gap-4">
        <Card>
          ...
          <div className="grid gap-3">
            <div className="rounded-lg border border-royal/20 bg-white p-3">
              <span className="text-xs font-bold uppercase text-slate-500">Último simulado</span>
        ...
        <Card className="taf-common-card">
          ...
          <p className="mb-4 text-sm text-gray-400">Os exercícios variam por cargo, estado e edital. Estes são os mais frequentes em concursos policiais.</p>
          <div className="grid gap-3">
            {COMMON_TAF.map((item) => (
        ...
        <Card>
          ...
          <div className="space-y-3 text-sm text-gray-300">
            ...
            <p><Dumbbell className="mr-2 inline text-blue-300" size={16} />2 dias de força para flexão, abdominal e barra.</p>
  ...
  return (
    <div className="taf-page">
      <div className="taf-header mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-col gap-4">
          <div>
            ...
            <p className="text-sm text-gray-400">Treino focado nos testes físicos de concursos: corrida, flexão, abdominal e barra.</p>
          </div>
        </div>
        <Badge variant="success">Exercícios com GIF</Badge>
      </div>
      <div className="taf-tabs" role="tablist" aria-label="Seções do TAF">
        {tabs.map(({ label, icon: Icon }) => (
          <button
            key={label}
            className={cx("taf-tab-button", tab === label && "is-active")}
            type="button"
            role="tab"
      ...
      <div className="taf-content mt-4">
        ...
        {tab === "Visão geral" ? <TAFOverview /> : null}
