...
function TrialExpiredGate() {
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-slate-50 px-4 py-8 text-slate-950">
      <section className="w-full max-w-5xl overflow-hidden rounded-2xl border border-royal/20 bg-white shadow-2xl shadow-blue-100/70">
        <div className="grid gap-0 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="border-r border-royal/20 bg-white p-7 text-slate-950 sm:p-10">
            <div className="grid size-14 place-items-center rounded-2xl bg-royal/10 text-royal">
              <LockKeyhole size={28} />
            </div>
            ...
            <div className="mt-6 rounded-xl border border-royal/20 bg-slate-50 p-4 text-sm text-slate-600">
              <ShieldCheck className="mb-2" size={20} />
              Seu progresso fica salvo. Assim que o pagamento for confirmado, o app libera novamente.
            </div>
            <button className="mt-6 inline-flex items-center gap-2 text-sm font-bold text-royal hover:text-navy" onClick={logout} type="button">
              <LogOut size={17} />
              Sair da conta
            </button>
          </div>
          ...
          <div className="grid gap-4 p-5 sm:p-7 lg:p-10">
            <div>
              <span className="text-xs font-black uppercase tracking-wide text-royal">Continuar no VemAprovar</span>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {Object.values(paymentPlans).map((plan) => (
                <article className="rounded-xl border border-royal/20 bg-white p-5 shadow-sm" key={plan.id}>
                  <span className="text-xs font-black uppercase text-royal">{plan.type}</span>
...
function MaintenanceGate({ message }) {
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-slate-50 px-4 py-8 text-center text-slate-950">
      <section className="w-full max-w-xl rounded-2xl border border-royal/20 bg-white p-8 shadow-2xl shadow-blue-100/70">
        <div className="mx-auto grid size-14 place-items-center rounded-2xl bg-royal/10 text-royal">
          <Wrench size={28} />
        </div>
        ...
        <button className="mt-6 text-sm font-bold text-royal hover:text-navy" onClick={logout} type="button">
          Sair da conta
        </button>
      </section>
    </main>
...
function isNewRegistration() {
  try {
    const session = JSON.parse(window.localStorage.getItem("aprovamais-session") || "null");
    return Boolean(session?.registrationPending);
  } catch {
    return false;
  }
}
...
function InternalRoutes() {
  ...
  if (!user?.onboardingComplete && isNewRegistration()) {
    return <Onboarding />;
  }
