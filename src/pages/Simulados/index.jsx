  ...
  return (
    <div>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-2">
        <Card><h2 className="mb-3 font-bold text-white">Acertos por materia</h2><DistributionPieChart data={result.porMateria} /></Card>
      ...
      <Card className="mt-4"><h2 className="mb-3 font-bold text-white">Questoes</h2><div className="grid gap-2 md:grid-cols-2">{result.questoes.map((item) => <div key={item.id} className="flex items-center justify-between rounded-lg bg-gray-900 p-3 text-sm text-gray-300"><span>{item.id} · {item.materia}</span>{item.correct ? <CheckCircle2 className="text-blue-300" /> : <XCircle className="text-red-300" />}</div>)}</div></Card>
    </div>
  );
}
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[1fr_260px]">
      <Card>
        ...
        <p className="rounded-lg bg-gray-900 p-4 text-gray-100">Questao {current + 1}: Em {question.materia}, analise o enunciado completo e escolha a alternativa correta conforme a banca.</p>
...
export default function SimuladosPage() {
  ...
  return <div><h1 className="mb-1 text-3xl font-black text-white">Simulados</h1><p className="mb-5 text-sm text-gray-400">Crie por edital, banca ou modo personalizado.</p><div className="grid gap-4 lg:grid-cols-3">{templates.map((template) => <Card key={template.id}><Badge>{template.modo}</Badge><h2 className="mt-3 text-xl font-bold text-white">{template.nome}</h2><div className="my-4 grid gap-3"><Select label="Materias" options={["Todas", "Direito Constitucional", "Portugues", "Informatica", "Raciocinio Logico", "Administrativo"]} /><Select label="Numero de questoes" options={["20", "30", "40", "60"]} /><Select label="Tempo" options={["2h", "3h", "4h"]} /></div><Button icon={Play} onClick={() => setActive(template)}>Iniciar</Button></Card>)}</div><div className="mt-4 grid gap-4 xl:grid-cols-2"><Card><h2 className="mb-3 font-bold text-white">Meus simulados</h2>{templates.map((item) => <div key={item.id} className="flex justify-between border-b border-gray-800 py-2 text-sm text-gray-300"><span>{item.nome}</span><Badge variant="success">concluido</Badge></div>)}</Card><Card><h2 className="mb-3 font-bold text-white">Evolucao</h2><PerformanceChart data={evolution} /></Card></div></div>;
}
