        ...
        try {
          ...
          extraContext = {
            ferramentaExecutada: {
              ...
              resultado: result,
              instrucao: "Avise que o plano foi atualizado diretamente no calendário e resuma as primeiras atividades criadas. Não diga que apenas sugeriu.",
            },
          };
        } catch (error) {
          extraContext = {
            ferramentaExecutada: {
              ...
              instrucao: "Avise que tentou atualizar o plano, mas não conseguiu salvar agora. Oriente o aluno a tentar novamente.",
            },
          };
        } finally {
          setActionLoading("");
    ...
    <div className="ai-chat mx-auto flex h-full min-h-[calc(100vh-150px)] w-full max-w-5xl flex-col rounded-lg border border-royal/20 bg-white shadow-sm">
      <div className="ai-chat-hero flex flex-col gap-4 border-b border-royal/20 bg-royal/10 p-4 text-slate-800 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="min-w-0">
          ...
          <span className="mt-2 inline-flex rounded-full bg-white px-2.5 py-1 text-xs font-black text-navy ring-1 ring-royal/30">
            {aiService.isConfigured ? "Aprovinho ativo" : "Aprovinho indisponivel"}
          </span>
          <span className="mt-2 inline-flex rounded-full bg-royal px-2.5 py-1 text-xs font-black text-white">
        ...
        <div className="mx-auto flex max-w-3xl flex-col gap-3">
          {messages.map((message, index) => (
            <div key={`${message.role}-${index}`} className={`flex gap-2 ${message.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[88%] whitespace-pre-wrap rounded-lg px-4 py-3 text-sm leading-relaxed shadow-sm ${message.role === "user" ? "bg-royal text-white" : "border border-royal/20 bg-white text-slate-800"}`}>
                {message.text}
              </div>
            </div>
          ))}
          ...
          {isStreaming ? (
            <div className="flex justify-start">
              <div className="max-w-[88%] whitespace-pre-wrap rounded-lg border border-royal/20 bg-white px-4 py-3 text-sm leading-relaxed text-slate-800 shadow-sm">
                {streamText || (
      ...
      <div className="ai-chat-prompts border-t border-royal/20 bg-white px-3 py-3 sm:px-5">
        ...
        {quickPrompts.map((chip) => (
          <button
            ...
            disabled={busy}
            onClick={() => send(chip)}
            className="shrink-0 rounded-full border border-royal/20 bg-royal/10 px-3 py-2 text-xs font-semibold text-navy hover:bg-royal hover:text-white disabled:cursor-not-allowed disabled:opacity-50"
            type="button"
          >
            {chip}
          </button>
        ...
        <button
          ...
          className="shrink-0 rounded-full bg-royal px-3 py-2 text-xs font-semibold text-white hover:bg-navy disabled:cursor-not-allowed disabled:opacity-50"
          type="button"
        >
      ...
      <div className="ai-chat-composer flex gap-2 border-t border-royal/20 bg-white p-3 sm:p-5">
        <Input
          value={input}
...
export const AIPanel = memo(({ text, action }) => (
  <div className="rounded-lg border border-blue-200 bg-royal/10 p-4 text-sm text-slate-700">
    <BookOpenCheck className="mb-2 text-navy" />
    {text}
